...
TransactionBuilder.prototype._selectUnspent = function(neededAmountSat) {
  ...
  if (!fulfill){
    console.log("not enough unspent tx outputs to fulfill totalNeededAmount [SAT]");
...
TransactionBuilder.prototype.setOutputs = function(outs) {
  console.log("this.vanilla.outs = JSON.stringify(outs);");
  this.vanilla.outs = JSON.stringify(outs);
  console.log("var valueOutSat = bignum(0);");
  var valueOutSat = bignum(0);
  console.log("var txobj = {};");
  var txobj = {};
  console.log("txobj.version = 1;");  
  txobj.version = 1;
  console.log("txobj.lock_time = this.lockTime || 0;"); 
  txobj.lock_time = this.lockTime || 0;
  console.log("var curTimestamp = Math.round(new Date().getTime() / 1000);");
  var curTimestamp = Math.round(new Date().getTime() / 1000);
  console.log("txobj.time = this.time || curTimestamp;");
  txobj.time = this.time || curTimestamp;
  console.log("txobj.ins = [];");
  txobj.ins = [];
  console.log("txobj.outs = [];");
  txobj.outs = [];

  console.log("var l = outs.length;");
  var l = outs.length;
  console.log("for (var i = 0; i < l; i++) {");
  for (var i = 0; i < l; i++) {
    console.log(" var amountSat = outs[i].amountSat || outs[i].amountSatStr ? bignum(outs[i].amountSatStr) : util.parseValue(outs[i].amount);");
    var amountSat = outs[i].amountSat || outs[i].amountSatStr ? bignum(outs[i].amountSatStr) : util.parseValue(outs[i].amount);
    console.log("var value = util.bigIntToValue(amountSat);");
    var value = util.bigIntToValue(amountSat);
    console.log("var script = TransactionBuilder._scriptForOut(outs[i]);");
    var script = TransactionBuilder._scriptForOut(outs[i]);
    console.log("var txout = {");
    console.log("  v: value,");
    console.log("  s: script.getBuffer(),");
    console.log("};");
    var txout = {
      v: value,
      s: script.getBuffer(),
    };
    console.log("txobj.outs.push(txout);");
    txobj.outs.push(txout);
    console.log("valueOutSat = valueOutSat.add(amountSat);");
    valueOutSat = valueOutSat.add(amountSat);
  }
  console.log("this.valueOutSat = valueOutSat;");
  this.valueOutSat = valueOutSat;
  console.log("this._setFeeAndRemainder(txobj);");
  this._setFeeAndRemainder(txobj);
  console.log("this.tx = new Transaction(txobj);");
